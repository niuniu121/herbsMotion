      ...
      >
        <div class="container">
          <div class="qna-shell">
            ...
            <div v-if="loading" class="qna-state-card">Loading questions and answers...</div>

            <div v-else-if="errorMessage" class="qna-state-card error">
              {{ errorMessage }}
            </div>

            <div v-else-if="pageContent.qnaItems.length === 0" class="qna-state-card">
              ...
              >
                ...
                <transition name="qna-expand">
                  <div v-if="openIndex === index" class="qna-answer">
                    <template
                      v-for="(block, blockIndex) in normaliseAnswerBlocks(item.answer)"
                      :key="blockIndex"
                    >
                      ...
                      <div v-else-if="block.type === 'subheading'" class="answer-subheading">
                        {{ block.text }}
                      </div>
      ...
      >
        <div class="container">
          <div class="qna-shell food-shell">
            ...
            <div v-if="pageContent.avoidGroups.length === 0" class="qna-state-card">
      ...
      >
        <div class="container">
          <div class="qna-shell lifestyle-shell">
            ...
            <div v-if="pageContent.lifestyleCards.length === 0" class="qna-state-card">
  ...
  qnaItems: [
    ...
    },
    {
      ...
      answer: [
        ...
        {
          type: 'list',
          items: ['Do not take on an empty stomach.', 'Do not take within 1 hour before sleep.'],
        },
      ],
    },
    {
...
const getSectionByType = (sections = [], type) => {
  return Array.isArray(sections) ? sections.find((section) => section.type === type) : null
}

const mergeWithDefaultContent = (remoteData) => {
  const remote = remoteData || {}

  const qnaSection = getSectionByType(remote.sections, 'qna')
  const foodSectionFromSections = getSectionByType(remote.sections, 'food')
  const lifestyleSectionFromSections = getSectionByType(remote.sections, 'lifestyle')

  const remoteQnaItems = Array.isArray(qnaSection?.items)
    ? qnaSection.items
    : Array.isArray(remote.qnaItems)
      ? remote.qnaItems
      : []

  const remoteAvoidGroups = Array.isArray(foodSectionFromSections?.items)
    ? foodSectionFromSections.items
    : Array.isArray(remote.avoidGroups)
      ? remote.avoidGroups
      : []

  const remoteLifestyleCards = Array.isArray(lifestyleSectionFromSections?.items)
    ? lifestyleSectionFromSections.items
    : Array.isArray(remote.lifestyleCards)
  ...
  return {
    ...
    foodSection: {
      ...
      ...(remote.foodSection || {}),
      ...(foodSectionFromSections
        ? {
            title: foodSectionFromSections.title || defaultPageContent.foodSection.title,
            description:
              foodSectionFromSections.description || defaultPageContent.foodSection.description,
          }
        : {}),
    },

    avoidGroups:
      remoteAvoidGroups.length > 0
        ? remoteAvoidGroups
    ...
    lifestyleSection: {
      ...
      ...(lifestyleSectionFromSections
        ? {
            title: lifestyleSectionFromSections.title || defaultPageContent.lifestyleSection.title,
            description:
              lifestyleSectionFromSections.description ||
              defaultPageContent.lifestyleSection.description,
          }
        : {}),
    },

    lifestyleCards:
      remoteLifestyleCards.length > 0
        ? remoteLifestyleCards
...
const fetchQnaPageContent = async () => {
  ...
  try {
    ...
    errorMessage.value = 'Failed to load QNA content. Showing default content for now.'
    pageContent.value = structuredClone(defaultPageContent)
  } finally {
    loading.value = false
  }
...
const convertStringAnswerToBlocks = (answer) => {
...
const normaliseAnswerBlocks = (answer) => {
  if (typeof answer === 'string') {
    return convertStringAnswerToBlocks(answer)
  }
